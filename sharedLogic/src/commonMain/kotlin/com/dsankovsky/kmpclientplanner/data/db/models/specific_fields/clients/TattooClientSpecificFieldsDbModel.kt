...
@Entity(
    tableName = "tattoo_client_specific_fields",
    foreignKeys = [
        ForeignKey(
            entity = BaseClientDbModel::class,
            parentColumns = arrayOf(BaseClientDbModel.Companion.NAME_ID),
            childColumns = arrayOf(TattooClientSpecificFieldsDbModel.NAME_CLIENT_ID),
            onDelete = ForeignKey.CASCADE
        )
    ]
)
data class TattooClientSpecificFieldsDbModel(
    @PrimaryKey(autoGenerate = true)
    @ColumnInfo(name = NAME_ID)
    val id: Long,
    @ColumnInfo(name = NAME_CLIENT_ID)
    val clientId: Long,
    val currentProject: TattooProjectDbModel?,
    val finishedProjects: TattooProjectsListDbModel?
) {

    @Serializable
    data class TattooProjectsListDbModel(
        val projects: List<TattooProjectDbModel>
    )

    @Serializable
    data class TattooProjectDbModel(
