...
- Кнопка «назад» в панели деталей появляется по `navigator.canNavigateBack()`: на широком
  окне обе панели видны и она не нужна, на compact панели показываются по одной.
- Выделение живёт в состоянии списка, а не в навигаторе: после удаления клиента `selectedClientId`
  пересчитывается на первого в списке, поэтому панель не остаётся на удалённой записи.
...
08, 09, 10 в кадре 1360×860 (`organic-screen-*.png`), 04/09/10 в узком кадре 720
...
| # | Экран | Что нужно |
|---|---|---|
| 05–07 | Детали услуги (репетитор / тренер / тату-бьюти) | 3 колонки: рейл 248 · список 400 (`surface@45%`, выбранная строка — `surface` + `shadow-md` + обводка accent) · детали. Специфика: репетитор — textarea «Домашнее задание»; тренер — таблица упражнений с колонкой «Прошлый раз» и дельта-тегом (+ М11); тату/бьюти — карусель референсов и сетка результата (+ М12). Каркас берётся тот же, что на 08 |
| М1/М3 | Формы услуги и клиента | Сейчас в модалке лежит старый экран со своим `Scaffold` (`LegacyFormModal`). Нужна форма по макету: сетка 1fr/1fr, статусные кнопки, разделитель и блок «Поля категории», деструктивное действие слева. Вместе с ней переезжают М6/М7/М8 внутри форм (в карточке клиента они уже на `ConfirmModal`) и появляется М9 |
...
| Что нужно | Где | Объём |
|---|---|---|
| ~~Выделение в master-detail: `selectedClientId`~~ — сделано вместе с 08 (плюс `searchQuery` и `clientsCount`); `selectedServiceId` — с 05–07 | `ClientsListScreenState`, `ServicesListScreenState` (+ actions) | S |
